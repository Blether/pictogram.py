...
def drawarrayoficons(numbersofeach, coloured_images, tile_horiz = 10):
  ...
  if total_N == 0:
    # deal with empty data by producing blank image of appropriate width with title
    tile_vert = 1
  else:
...
def addtitle(icon_image, title, v_pad=20, bg='white', height=0):
  dim_hor, dim_vert = icon_image.size
  ...
  if height == 0:
    ...
    # allow forcing of height if wanted
...
def getmaxtitleheight(titles, font = ImageFont.truetype('Helvetica', 64)):
  message = str(titles) # may not be perfect but seems to work
  maxheight = font.getsize(message)[1]
  return maxheight
...
def appendright(left_image, right_image, h_pad=0, bg='white'):
  ...
  appended_image.paste(right_image, (w_l+h_pad, 0))
  # appended_image.paste(right_image, (w_l+h_pad, height-h_r))
  return appended_image

def makearray(numbersofeach, coloured_images, outcomes, title = '', titleheight = 0):
  ...
  if title is not '':
    out_image = addtitle(out_image, title, height=titleheight)
  return out_image
...
def pictogram(numbersofoutcome, icon_colours, outcomes, titles, dimension=64, dimension_vert=92, n_horiz=10, h_pad=20, bg='white', iconshape = 'person' ):
  ...
  titleheight = getmaxtitleheight(titles)
  
  arrayofnumbers = np.array(numbersofoutcome)
  separate_images = []
  if arrayofnumbers.ndim > 1:
    for each in range(arrayofnumbers.shape[0]):
      separate_images.append(makearray(arrayofnumbers[each,].tolist(), coloured_images, outcomes, titles[each], titleheight))
  else:
    separate_images.append(makearray(arrayofnumbers.tolist(), coloured_images, outcomes, titles, titleheight))
  # append these
...
def pictogramNRR(numbersofoutcome, icon_colours, outcomes, titles, dimension=64, dimension_vert=92, n_horiz=10, h_pad=20, bg='white', iconshape = 'person' ):
  ...
  if arrayofNRR.ndim > 1:
    for each in range(arrayofNRR.shape[0]): # this was broken before
      ...
      separate_images.append(makearray(numberstouse, coloured_images, outcomes, titles[each], titleheight))
  else:
    numberstouse = np.append(differences, arrayofNRR[-1]).tolist()
    separate_images.append(makearray(numberstouse, coloured_images, outcomes, titles, titleheight))
  # append these
  output_image = separate_images[0]
  if arrayofNRR.ndim > 1:
...
def main():
  ...
  complex.show()
  morecomplex = pictogramNRR([[100,58,13,7], [0,0,0,0], [100,42,14,2]] , ['grey', 'cyan', 'lightgreen', 'green'], ['', 'slight', 'improvement', 'cure'], ['magic', 'summat', 'nowt'])
  morecomplex.show()
